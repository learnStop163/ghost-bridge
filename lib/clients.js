...
export function getClientDefinitions() {
  ...
  const hasClaudeLegacy = fs.existsSync(claudeLegacyPath);

  return [
    getJsonClientDefinition('Claude Code (~/.claude.json)', claudeLegacyPath, {
      shouldCreate: hasClaudeLegacy || fs.existsSync(claudeDir),
      isAvailable: () => hasClaudeLegacy || fs.existsSync(claudeDir)
    }),
    {
      name: 'Codex',
      kind: 'toml',
